class Connection {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < this.benchmark_obj.request_interval; i++) {
                ...
                if (this.client !== undefined) {
                    ...
                    value = 48 + i;
                    this.writer[i].write(new Uint8Array([123, 34, 99, 34, 58, value, 125]));
    ...
    async listen(i) {
        const self = this;
        self.received = await self.reader[i].read();
        self.listener[i] = self.received.value;
//	console.log("Message received back ",self.listener[i]);
        self.times[i]['received']=Date.now();
        self.times[i]['finish'] = Date.now();
    ...
    async connect(url) {
        ...
        for( let i=0;i<this.benchmark_obj.request_interval;i++) {
            this.stream[i] = await this.client.createBidirectionalStream();
            this.reader[i] = this.stream[i].readable.getReader();
            this.writer[i] = this.stream[i].writable.getWriter();
            //console.log("End of connect ", this.writer);
            await this.writer[i].write(new Uint8Array([123, 34, 99, 34, 58, 48, 125]));
            received = await this.reader[i].read();
        }
